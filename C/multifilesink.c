 ...
 * 15/08/2016
 *
 * gcc multifilesink.c -o multifilesink `pkg-config --cflags --libs gstreamer-1.0`
 */
...
int main(int argc, char *argv[]) {
  ...
  /* Create the elements */
  data.pipeline = gst_parse_launch ("v4l2src name=src ! video/x-raw,framerate=24/1,width=640 ! queue ! videoconvert ! x264enc threads=0 bitrate=400 tune=zerolatency ! avimux ! multifilesink name=sink next-file=3 location=out%d.avi", NULL);
